...
function startPopupNews() {
    try {
        ...
        console.debug("startPopupNews: " + err);
    }
}
...
const newsPopup = {
    ...
    checkVersionBetween: function (inst, vers1, vers2) {
        return (
            inst === vers1 ||
            inst === vers2 ||
            (newsPopup.checkVersion(vers1, inst) && newsPopup.checkVersion(inst, vers2))
        );
    },
    ...
    checkMessages: async function (obj, instAdapters) {
        ...
        try {
            ...
            if (messages.length > 0) {
                await asyncForEach(messages, async function (message) {
                    ...
                    if (showIt && message["node-version"]) {
                        showIt =
                            systemInformationData.node &&
                            newsPopup.checkConditions(message["node-version"], systemInformationData.node);
                    }
                    if (showIt && message["npm-version"]) {
                        showIt =
                            systemInformationData.npm &&
                            newsPopup.checkConditions(message["npm-version"], systemInformationData.npm);
                    }
            ...
            console.debug("checkMessages: " + err);
        }
    ...
    showDiv: function (id, title, content, type, icon, appendId, uniqueId) {
        ...
        if (id && $("#popupnewsid_" + id).length === 0 && !ignored) {
            ...
            $item
                .find(".popupnews_title")
                .text(title)
                .attr("id", "popupnewsid_" + id);
    ...
    getAdaptersAndcheckMessages: function (obj, toSetId, dummy) {
        try {
            ...
            console.debug("getAdaptersAndcheckMessages: " + err);
        }
    },
};
