        ...
        if (rssFeed.length > 0) {
            for (let i = 0; i < 30; i++) {
                ...
                desc +=
                    ...
                    _("replied on") +
                    " " +
                    new Date(thread.pubDate).toLocaleDateString(systemLang, dateOptions) +
                    ":</b>";
                ...
                $item
                    .find(".byline")
                    .text(new Date(feed.pubDate).toLocaleDateString(systemLang, dateOptions) + " - " + feed.author);
            ...
            if (e == "Error: Hostname in referer header is not registered") {
                $("#forumList").append(
                    $(
                        "<li>" +
                            _(
                                "To read news or forum, you need a free API Key if your ioBroker installation can be reached via a hostname, such as iobroker: 8081 or something similar. For IP access it is not needed.",
                            ) +
                            "</li>",
                    ),
                );
