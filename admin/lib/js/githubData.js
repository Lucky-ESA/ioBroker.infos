...
const githubHelper = {
    ...
    createIssue: function () {
        ...
        if (issueTitle.length > 0 && issueBody.length > 0 && url.length > 0) {
            ...
            if (!url.length > 0) {
                $("#githubErrorList").append(
                    $("<li>" + _("Something went wrong. Please close the window and open again.") + "</li>"),
                );
            }
            $("#githubErrorList").parent().removeClass("hidden");
        }
    ...
    getQueryForIssues: function (owner, name, login, isAdapterRequest, cursor, search, onlyOpen = true) {
        ...
        if (search) {
            ...
            if (login) {
                query = query
                    .replace("$repoORuser", 'user(login: "' + login + '"){')
                    .replace("$reactions", "")
                    .replace("$orderby", ", orderBy:{field: CREATED_AT, direction: DESC}");
            } else {
                query = query.replace("$repoORuser", 'repository(owner: "' + owner + '", name: "' + name + '") {');
                if (isAdapterRequest) {
                    query = query
                        .replace("$reactions", "reactions(first: 100) {totalCount viewerHasReacted}")
                        .replace("$orderby", "");
                } else {
                    query = query
                        .replace("$reactions", "")
    ...
    setReaction: async function (id) {
        ...
        const response = await githubHelper.getData(
            "https://api.github.com/repos/ioBroker/AdapterRequests/issues/" + issueNumber + "/reactions",
            "POST",
            { content: "+1" },
        );
        if (response && response.id) {
            ...
            sessionStorage.removeItem("ioBroker.infos.adapterRequestV4");
        }
    },
    setStarred: async function (id) {
        ...
        if (!response) {
            $("#" + id)
                .removeClass("btn-default")
                .addClass("btn-success")
                .attr("title", _("Thanks for the adapter!"));
            let count = parseInt($("#starsCounter" + idSuffix).text()) + 1;
            $("#starsCounter" + idSuffix)
                .addClass("badge-success")
                .text(count)
                .parent()
                .parent()
                .css("background-color", "#dff0d8");
            sessionStorage.removeItem("ioBroker.infos.stargazers");
        }
    },
    ...
    writeComments: function (issueId, comments) {
        comments.forEach(function (commentNode) {
            ...
            $item
                .find(".byline")
                .text(new Date(comment.createdAt).toLocaleDateString("en", dateOptions) + " - " + comment.author.login);

            const link = comment.url.match(/([^/]*\/){6}/);
            const html = new showdown.Converter()
                .makeHtml(comment.body)
