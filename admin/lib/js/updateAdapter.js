...
const getAdaptersInfo = function (host, callback) {
    ...
    if (!this.curRepository) {
        socket.emit(
            ...
            host,
            "getRepository",
            { repo: systemConfig.common.activeRepo, update: true },
    ...
    if (this.curInstalled && this.curRepository) {
        setTimeout(function () {
            ...
            if (callback) callback(curRepository, curInstalled);
        }, 0);
    } else {
        curRunning = [callback];
    }
...
function fillList(type, list, repository, installedList) {
    ...
    for (let i = 0; i < list.length; i++) {
        const $tmpLiElement = $("#" + type + "HomeListTemplate")
            .children()
            .clone(true, true);
        ...
        if (isHost) {
            ...
            $tmpLiElement
                .find(".title")
                .text((obj.title || obj.titleLang[systemLang]).replace("ioBroker Visualisation - ", ""));
        }
        ...
        if (isHost) {
            ...
            $tmpLiElement
                .find(".host-readme-submit")
                .attr(
                    "data-md-url",
                    obj.readme.replace("https://github.com", "https://raw.githubusercontent.com").replace("blob/", ""),
                );
    ...
    if (isInstalled && installedList) {
        if (counter === 0) {
            $("#homeUpdateListTab")
                ...
                .addClass("allOk")
                .html(
                    '<h3 id="noUpdateAllOk" style="text-align: center;">' + _("All adapters are up to date!") + "</h3>",
                );
        }
    }
