...
async function getNodeVersionList(callback) {
    let data;
    if (sessionStorage.getItem("ioBroker.infos.nodejsInfo")) {
        data = JSON.parse(sessionStorage.getItem("ioBroker.infos.nodejsInfo"));
    } else {
        data = await (await fetch("https://nodejs.org/dist/index.json")).json();
        sessionStorage.setItem("ioBroker.infos.nodejsInfo", JSON.stringify(data));
    }
...
function getNodeExtrainfo(host) {
    const hostID = host.replace(/[^a-z0-9]/gim, "_").replace(/\s+/g, "_");
    ...
    if (infoData.nodeOld.indexOf(aktKey) !== -1) {
        extraInfo +=
            ...
            _("Node.js too old") +
            " " +
            versionMap[infoData.nodeRecommended] +
            "</span>";
        color = "red";
