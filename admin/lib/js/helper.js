...
async function readInstanceConfig(callback) {
    ...
    if (sessionStorage.getItem("ioBroker.infos.documentationData")) {
        documentationData = JSON.parse(sessionStorage.getItem("ioBroker.infos.documentationData"));
    } else {
        try {
            documentationData = await (
                await fetch("https://raw.githubusercontent.com/ioBroker/ioBroker.docs/master/info/documentation.json")
            ).json();
            sessionStorage.setItem("ioBroker.infos.documentationData", JSON.stringify(documentationData));
        } catch (e) {}
    }

    socket.emit("getState", "infos.0.sysinfo.os.versions.node", function (err, data) {
        if (!err && data) {
            systemInformationData.node = data.val;
        }
    });
    socket.emit("getState", "infos.0.sysinfo.os.versions.npm", function (err, data) {
        if (!err && data) {
            systemInformationData.npm = data.val;
        }
    });
    socket.emit("getState", "infos.0.sysinfo.os.info.platform", function (err, data) {
        if (!err && data) {
            systemInformationData.os = data.val;
        }
    });
    socket.emit("getState", "infos.0.uuid", function (err, data) {
        if (!err && data) {
            systemInformationData.uuid = data.val;
        }
    });

    socket.emit("getState", "infos.0.ignored_news", function (err, data) {
        if (!err && data) {
            try {
    ...
    socket.emit("getObject", "system.config", function (err, data) {
        ...
        socket.emit("getObject", "system.adapter.infos.0", function (err, res) {
...
$(function () {
    ...
    $(document.body).on("click", ".loadGithubData", function () {
        ...
        $.get(url, function (data) {
            ...
            const html = new showdown.Converter()
                .makeHtml(data)
                .replace(/src="(?!http)/g, 'class="img-responsive" src="' + link[0]);
            $("#" + target).html(html);
        }).fail(function () {
            $("#" + target).html(
                "<p><b>" + _("Readme file was not found. No further information available.") + "</b></p>",
            );
        });
    });
    ...
    $('a[href="#sys_info_tab_os"]')
        .on("show.bs.tab", () => {
            socket.emit("subscribe", "infos.0.sysinfo.os.*");
        })
        .on("hide.bs.tab", () => {
            socket.emit("unsubscribe", "infos.0.sysinfo.os.*");
        });
    $('a[href="#sys_info_tab_cpu"]')
        .on("show.bs.tab", () => {
            socket.emit("subscribe", "infos.0.sysinfo.cpu.*");
        })
        .on("hide.bs.tab", () => {
            socket.emit("unsubscribe", "infos.0.sysinfo.cpu.*");
        });
    $('a[href="#sys_info_tab_memory"]')
        .on("show.bs.tab", () => {
            socket.emit("subscribe", "infos.0.sysinfo.memory.*");
        })
        .on("hide.bs.tab", () => {
            socket.emit("unsubscribe", "infos.0.sysinfo.memory.*");
        });
    $('a[href="#sys_info_tab_disks"]')
        .on("show.bs.tab", () => {
            socket.emit("subscribe", "infos.0.sysinfo.disks.*");
        })
        .on("hide.bs.tab", () => {
            socket.emit("unsubscribe", "infos.0.sysinfo.disks.*");
        });
    $('a[href="#sys_info_tab_battery"]')
        .on("show.bs.tab", () => {
            socket.emit("subscribe", "infos.0.sysinfo.battery.*");
        })
        .on("hide.bs.tab", () => {
            socket.emit("unsubscribe", "infos.0.sysinfo.battery.*");
        });
    $('a[href="#sys_info_tab_usb"]')
        .on("show.bs.tab", () => {
            socket.emit("subscribe", "infos.0.sysinfo.usb.*");
        })
        .on("hide.bs.tab", () => {
            socket.emit("unsubscribe", "infos.0.sysinfo.usb.*");
        });
    $('a[href="#sys_info_tab_bluetooth"]')
        .on("show.bs.tab", () => {
            socket.emit("subscribe", "infos.0.sysinfo.bluetooth.*");
        })
        .on("hide.bs.tab", () => {
            socket.emit("unsubscribe", "infos.0.sysinfo.bluetooth.*");
        });

    $("#modal-system").on("hidden.bs.modal", () => {
        socket.emit("unsubscribe", "infos.0.sysinfo.*");
    });
    ...
    $(document.body).on("click", ".popupnewsneveragain", function () {
        ...
        ignoredNews.push(id);
        socket.emit("setState", "infos.0.ignored_news", { val: JSON.stringify(ignoredNews), ack: true });
    });
