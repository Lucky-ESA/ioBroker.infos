...
async function getAllIssues(owner, name, login, search, onlyOpen) {
    ...
    if (issues && issues.data && (issues.data.repository || issues.data.user || issues.data.search)) {
        ...
        while (hasNext) {
            const nextQL = githubHelper.getQueryForIssues(
                owner,
                name,
                login,
                isAdapterRequest,
...
async function getAllIssuesFromAdapter(full_name) {
    ...
    for (let i = 1; i < 20; i++) {
        const issues = await (
            await fetch("https://api.github.com/repos/" + full_name + "/issues?page=" + i + "&per_page=100")
        ).json();
        ...
        allIssues.forEach(function (issueNode) {
            ...
            if (id === "adapterRequestList") {
                ...
                if (issue.comments.totalCount > 0) {
                    buttons +=
                        "<button type='button' data-issue-id='" +
                        issue.id +
                        "' id='issueComments" +
                        issue.databaseId +
                        "' class='openIssueComments btn btn-default'><i class='fa fa-commenting fa-lg'></i></button>&nbsp;";
                }
                if (issue.state === "OPEN") {
                    buttons +=
                        "<button type='button' title='" +
                        (issue.reactions.viewerHasReacted ? _("I voted!") : _("Vote for this adapter request!")) +
                        "' id='reactionAR" +
                        issue.number +
                        "' class='adapterRequestReaction btn btn-" +
                        (issue.reactions.viewerHasReacted ? "success" : "default") +
                        "'><i class='fa fa-thumbs-up fa-lg'></i></button>";
                } else if (issue.reactions.viewerHasReacted) {
                    buttons +=
                        "<button type='button' title='I voted!' class='btn btn-success'><i class='fa fa-thumbs-up fa-lg'></i></button>";
                }
                buttons += "</div>";
                ...
                $item
                    .find(".y_content")
                    .append($(votes))
                    .append($("<div class='marginRunter' id='allCommentsDiv" + issue.databaseId + "'></div>"));
            } else {
                if (issue.comments.totalCount > 0) {
                    ...
                    const comment =
                        "<span class='comments-counter has-badge' data-count='" +
                        number +
                        "'><i class='fa fa-comment xfa-inverse'></i></span>";
                    ...
                    const commentsCount =
                        ...
                        "</span></strong>";
                    const allComments =
                        "<div class='pull-right marginHoch'><button type='button' data-issue-id='" +
                        issue.id +
                        "' id='issueComments" +
                        issue.databaseId +
                        "' class='openIssueComments btn btn-default'><i class='fa fa-commenting fa-lg'></i></button></div>";
                    $item
                        .find(".y_content")
            ...
            $item
                .find(".byline")
                .text(
                    createdAt.toLocaleDateString("en", dateOptions) + " - " + (issue.author ? issue.author.login : ""),
                );
...
function showAdapterRequest() {
    async function getIssues() {
        let allIssues;
        const storagePoint = adapterConfig.github_token
            ? "ioBroker.infos.adapterRequestV4"
            : "ioBroker.infos.adapterRequest";
        if (sessionStorage.getItem(storagePoint)) {
            ...
            if (adapterConfig.github_token) {
                allIssues = await getAllIssues(
                    ...
                    null,
                    false,
                    adapterConfig.adapter_request_close_hidden ? true : false,
                );
            } else {
