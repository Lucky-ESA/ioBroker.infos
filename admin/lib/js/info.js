...
const updateInfoPage = async function () {
    ...
    for (let currentHost in hosts) {
        getHostInfo(hosts[currentHost], function (data) {
            let text = "";
            if (data) {
                const hostID = data.hostname.replace(/[^a-z0-9]/gim, "_").replace(/\s+/g, "_");
                ...
                for (let item in data) {
                    if (data.hasOwnProperty(item)) {
                        ...
                        if (item === "Node.js") {
                            ...
                            text +=
                                '<dd><span id="aktNodeVersion' +
                                hostID +
                                '">' +
                                (formatInfo[item] ? formatInfo[item](data[item]) : data[item]) +
                                '</span><span id="nodeExtraInfo' +
                                hostID +
                                '"></span></dd>';
                        } else {
                            if (item === "NPM") {
                            ...
                            text +=
                                "<dd" +
                                (item === "Uptime" || item === "System uptime"
                                    ? " id='" + hostID + item + "' class='timeCounter' data-start='" + data[item] + "'"
                                    : "") +
                                ">" +
                                (formatInfo[item] ? formatInfo[item](data[item]) : data[item]) +
                                "</dd>";
    ...
    readInstanceConfig(async function () {
        ...
        if (adapterConfig.hide_events) {
            ...
            formatGoogleCalendar.init({
                calendarUrl:
                    "https://cors-anywhere.herokuapp.com/https://www.googleapis.com/calendar/v3/calendars/mh14bh7m2bdva7pb7tkirsbcsg@group.calendar.google.com/events?key=AIzaSyA4GWhsN21aMARuc6uyl45zdq8Ue5dJu10",
                past: false,
                upcoming: true,
                sameDayTimes: true,
                dayNames: false,
                ...
                upcomingHeading:
                    "<h4>" +
                    _("Upcoming events") +
                    ' <a href="javascript:;" onclick="$(\'#modal-calendar\').modal({show: true});"><i class="fa fa-calendar" aria-hidden="true"></i></a></h4>',
                pastHeading: "<h4>" + _("Past events") + "</h4>",
                format: [
                    '<div class="block"><div class="block_content"><div class="y_title spoiler-content" style="padding-left: 20px;"><ul class="nav navbar-right panel_toolbox"><li><a class="collapse-link"><i class="fa fa-chevron-down"></i></a></li></ul><span>',
                    "*date*",
                    ": ",
                    "*summary*",
                    '</span></div><div class="y_content spoiler-content" style="display: none;"><p class="description">',
                    "*description*",
                    " in ",
                    "*location*",
                    "</p></div></div></div>",
